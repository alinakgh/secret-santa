...
export default class CardModal extends React.Component {
  ...
  _showCurrencyPicker = () => {
    getFullLabel = currency => {
      return currency.label.concat(
        " (",
        getSymbolFromCurrency(currency.abv),
        ")"
      );
    };
    ...
    return (
      <Card containerStyle={styles.cardNoMargin}>
        <TouchableOpacity
          ...
          onPress={this._toggleCurrencyPicker}
        >
          ...
          <View style={{ paddingTop: 30 }}>
            <View style={styles.budgetAdjust}>
              ...
              <View style={styles.budgetView}>
                <Text size="20" style={{ textAlign: "center", fontSize: 18 }}>
                  {this.state.input.budget}
                </Text>
              </View>
            ...
            <Picker
              ...
              onValueChange={(itemValue, itemIndex) =>
                this._setCurrency(itemValue)
              }
            >
              {currencies.map((currency, i) => (
                <Picker.Item
                  key={currency.abv}
                  label={getFullLabel(currency)}
                  value={currency.abv}
                />
              ))}
            </Picker>
  ...
  _showPictureChoice = () => {
    ...
    isCustom = idx => {
      if (idx === 2) {
        return <Icon name="camera" type="entypo" />;
      }
    };
    ...
    return (
      <Card containerStyle={styles.cardNoMargin}>
        <TouchableOpacity
          style={styles.cardContent}
          onPress={this._togglePictureChoice}
        >
          <View>
        ...
        <Collapsible collapsed={!this.state.isVisible.pictureChoice}>
          <View>
            {picturesArr.map((pic, i) => (
              ...
              >
                <TouchableOpacity
                  onPress={() => this._setPictureOption(i)}
                  style={{
                    flexDirection: "row",
